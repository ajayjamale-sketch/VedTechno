            ...
            {filteredJobs.length > 0 ? (
              filteredJobs.map(job => (
                <div key={job.id} className="group flex flex-col sm:flex-row sm:items-center justify-between p-6 bg-card border border-border rounded-2xl hover:border-primary/30 hover:shadow-md transition-all">
                  ...
                  <button onClick={() => { setSelectedJob(job.role); setApplyModalOpen(true); }} className="mt-4 sm:mt-0 px-6 py-2.5 bg-secondary text-foreground text-sm font-medium rounded-xl hover:bg-primary hover:text-white transition-colors">
                    Apply Now
                  </button>
                </div>
              ))
